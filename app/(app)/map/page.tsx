...
//   const { saveScrollPosition, clearScrollPosition } = useScrollPreservation();
...
//     };
//     const existing = sessionStorage.getItem("restaurantFilters");
//     let merged: Record<string, unknown> = { ...filterData };
//     if (existing) {
//       try {
...
//         };
//       } catch {
//         /* keep filterData only */
//       }
//     }
//     sessionStorage.setItem("restaurantFilters", JSON.stringify(merged));
//   }, [filterState]);
...
//         router.push(`/sign-up?returnTo=${encodeURIComponent(returnPath)}`);
//         return;
//       }
...
//       return filterState.selectedCuisines.join(", ");
//     }
...
//     return (
//       <div className={cn(compact ? "space-y-2" : "space-y-4 md:space-y-6")}>
//         {(filterState.selectedMealTimes.length > 0 ||
...
//               </span>
//             </div>
//             <div className="relative z-0 isolate h-[min(38vh,240px)] min-h-[200px] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm ring-1 ring-black/[0.04] sm:h-64 sm:min-h-[240px] md:h-72 lg:h-80 max-md:h-full max-md:min-h-0 max-md:flex-1 max-md:rounded-none max-md:border-0 max-md:ring-0 max-md:shadow-none">
//               <UserLocationMap
...
import { RestaurantPageClient } from "@/app/(app)/restaurant/[id]/restaurant-page-client";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
/** Mobile map drawer snap fractions (vaul: fraction of viewport height). */
const MOBILE_RESTAURANTS_DRAWER_PEEK = 0.12;
const MOBILE_RESTAURANTS_DRAWER_EXPANDED = 0.9;
...
interface UIState {
  ...
  showAllCuisines: boolean;
  unlockModalRestaurant: Restaurant | null;
  selectedRestaurantIdForDeal: string | null;
}
...
export default function MapPage() {
  ...
  const [uiState, setUIState] = useState<UIState>({
    ...
    showAllCuisines: false,
    unlockModalRestaurant: null,
    selectedRestaurantIdForDeal: null,
  });
  ...
  const handleRestaurantNavigate = useCallback(
    async (restaurantPathSegment: string, offerId?: string) => {
      setUIState((prev) => ({ ...prev, selectedRestaurantIdForDeal: restaurantPathSegment }));
    },
    [],
  );
                            ...
                            <div className="flex items-start justify-between">
                              ...
                              >
                                {favoritesLoading.has(restaurant.id) ? (
                                  ...
                                  <Heart
                                    className={`h-4 w-4 ${favorites.has(restaurant.id)
                                        ? "fill-[#eb221c]"
                                        : ""
                                      }`}
                                  />
  ...
  return (
    <>
      <main className="min-h-screen bg-[#FFFBF7] pb-20 max-md:min-h-[100dvh] max-md:pb-0">
        ...
        {isMobile && (
          <Drawer
            open={!uiState.selectedRestaurantIdForDeal}
            onOpenChange={() => undefined}
            dismissible={false}
            modal={false}
            shouldScaleBackground={false}
        ...
        {uiState.selectedRestaurantIdForDeal && (
          <Dialog
            open={!!uiState.selectedRestaurantIdForDeal}
            onOpenChange={(open) => {
              if (!open) {
                setUIState((prev) => ({ ...prev, selectedRestaurantIdForDeal: null }));
              }
            }}
          >
            <DialogContent className="max-w-xl w-[95vw] sm:w-[90vw] md:w-full p-0 overflow-hidden bg-transparent border-0 shadow-none max-h-[90vh] flex flex-col [&>button]:hidden sm:rounded-2xl">
              <RestaurantPageClient 
                routeParam={uiState.selectedRestaurantIdForDeal} 
                isModal={true} 
                onClose={() => setUIState((prev) => ({ ...prev, selectedRestaurantIdForDeal: null }))}
              />
            </DialogContent>
          </Dialog>
        )}
