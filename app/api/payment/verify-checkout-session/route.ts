...
export async function POST(request: Request) {
  try {
    ...
    let stripeTrialEnd: number | null = null;
    ...
    if (session.subscription) {
      ...
      if (subscriptionInfo.trial_end) {
        stripeTrialEnd = subscriptionInfo.trial_end;
      }
    ...
    // 30 days from now only when Stripe doesn't provide one.
    const trialEndDate = new Date();
    if (stripeTrialEnd) {
      trialEndDate.setTime(stripeTrialEnd * 1000);
    } else {
