...
export default function RestaurantDetailPage({ params }: any) {
  ...
  const { nsr, id }: any = React.use(params);
  ...
  const fetchRestaurantDetails = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/admin/restaurants/${nsr}`)
  ...
  const handleStatusChange = async (newStatus: string) => {
    try {
      ...
      const response = await fetch(`/api/admin/restaurants/${nsr}/status`, {
        method: "PUT",
        headers: {
      ...
      // Update local state
      setRestaurant((prev: any) => (prev ? { ...prev, status: newStatus } : null))
  ...
  const handleOfferStatusChange = async (newStatus: string) => {
    try {
      ...
      if (newStatus === "active" && selectedAreas.length === 0) {
        toast.error("Please select at least one area before approving the offer.");
        setLoadingApprove(false); // Stop loading for approval
        return;
      }

      // Call the status update API
      const response = await fetch(`/api/restaurant/offers/${selectedOffer._id}/status`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const updatedOffersResponse = await fetch(`/api/admin/restaurants/${nsr}`);
  ...
  const handleAssignAreas = async () => {
    try {
      const response = await fetch(`/api/restaurant/offers/${selectedOffer._id}/assignarea`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    return (
      <div className="py-8">
        ...
        <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-md">
          ...
          <Button onClick={() => router.push("/admin/associated-restaurants")} className="mt-4" variant="outline">
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <CardDescription>
                    ...
                    {restaurant.area
                      .map((areaId: string) => areas.find((area: any) => area.value === areaId)?.label)
            ...
            <CardContent>
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                <TabsList className="mb-4">
                  ...
                  <TabsTrigger value="photos">Photos ({restaurant.images?.length || 0})</TabsTrigger>
                </TabsList>
          ...
          View Menu {restaurant.menuPdfUrls.length > 1 ? `#${index + 1}` : ""}
                ...
                <TabsContent value="offers">
                  {isEditingOffer ? (
                    <>
                      <AdminEditOfferPage
                        params={{ id: nsr, offerId: editingOfferId || undefined }}
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button variant="outline" className="w-full" asChild>
                <Link href={`/admin/associated-restaurants/${id}/edit/${restaurant._id}`}>Edit Restaurant Details</Link>
              </Button>
            </CardContent>
          </Card>
          <Card className="mt-6">
              ...
              <Button
                ...
                onClick={async () => {
                  if (selectedAreas.length === 0) {
                    toast.error("Please select at least one area before saving.");
                    return;
                  }
          ...
          <Card className="mt-6">
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold">{offers.filter((o: any) => o.status === "active").length}</span>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold">{new Date(restaurant.createdAt).toLocaleDateString('en-GB')}</span>
              </div>
            </CardContent>
          </Card>
        </div>
