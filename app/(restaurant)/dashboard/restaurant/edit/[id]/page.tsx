...
export default function RestaurantEditPage() {
  ...
  useEffect(() => {
    ...
    if (zip.length < 5) {
      setFormData((prev: any) => ({ ...prev, lat: undefined, lng: undefined }));
      return;
    }
  ...
  useEffect(() => {
    const fetchTags = async () => {
      try {
        const res = await fetch("/api/admin/tags?active=true")
  ...
  useEffect(() => {
    const fetchRestaurantData = async () => {
      try {
        ...
        setSelectedTags(
          Array.isArray(data.searchTags) ? data.searchTags : []
        );
        setUploadedImages(data.images || []);
        setIsLoadingData(false);
        setSelectedAreas(data.area || []);
        setMenuPdfUrls(data.menuPdfUrls || []);
  ...
  const removeImage = async (index: number) => {
    try {
      if (index === 0) {
        toast.error(
          "Default image cannot be removed. Please set another image as default first."
        );
        return;
      }
      ...
      if (!imageUrl.includes("raffilybusiness")) {
        // await deleteFileFromAzure(imageUrl);
      ...
      const data = await res.json();
      if (!res.ok || !data.success) {
        toast.error(data.message || "Failed to remove image from database");
        return;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/restaurant", {
        ...
        body: JSON.stringify({ ...formData, area: selectedAreas, restaurantId: id, menuPdfUrls, searchTags: selectedTags, }),
      })
              ...
              <CardContent className="space-y-4">
                ...
                {/* <div className="space-y-3 mt-6">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Add relevant tags to help customers find your restaurant
                  </p>
                  ...
                  <div className="flex gap-2">
                    <Input
                      placeholder="Add custom tag (e.g., Live Music)"
                      value={customTag}
