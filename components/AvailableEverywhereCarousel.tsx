...
interface AvailableEverywhereCarouselProps {
...
export function AvailableEverywhereCarousel({
  selectedLocationId,
  areasList,
  isAuthenticated,
  onUnlockClick,
  ...
  searchTerm = "",
  onVisibilityChange,
}: AvailableEverywhereCarouselProps) {
  ...
  const fetchRestaurants = useCallback(async (page: number, reset: boolean = false) => {
    ...
    try {
      const params = new URLSearchParams({
        page: page.toString(),
        limit: '12',
        // No categoryId - show all restaurants
      })

      // Only apply area filter if selected (this is the key difference)
      ...
      console.error('Error fetching restaurants for Available Everywhere:', error)
    } finally {
      setLoading(false)
  ...
  }, [selectedLocationId, selectedDiningStr, selectedDayValuesStr, selectedMealTimesStr, searchTerm])
